package com.example.storytell.init.util;

public class CustomPerlinNoiseGenerator { // Perlin noise generator by alksily
    private int[] p = new int[512];

    public CustomPerlinNoiseGenerator() {
    ...
    public double noise(double x, double seed, double size) {
        double value = 0.0;
        double initialSize = size;

        while (size >= 1) {
            value += smoothNoise((x / size), (0f / size), (0f / size), seed) * size;
            size /= 2.0;
        }
    ...
    public double smoothNoise(double x, double y, double z, double seed) {
